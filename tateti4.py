...
def movement(usuario):
    ...
    while esta_ocupado == True:
        while True:
            try:
                moveRow = input(nombre_usuario + " escoja la fila en donde ubicará la próxima ficha (1 a 3): ")
                if moveRow.isdigit()== True:
                    if int(moveRow) >= 1 and int(moveRow) <= 3:
                        break
            except KeyboardInterrupt:
                print("El juego se ha interrumpido.")
                sys.exit()
        while True:
            try:
                moveCol = input(nombre_usuario + " escoja la columna en donde ubicará la próxima ficha (1 a 3): ")
...
# Ejecutamos los 9 movimientos y determinamos si hay ganador o empate:
for i in range(9):    
    if i % 2 == 0:
        movement(primero)
        ...
        if ganador == True:
            break
    if i == 8:
        print("La partida ha terminado en empate. Bien jugado.")
        break
